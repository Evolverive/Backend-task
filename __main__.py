...
def sorting(category):
    ...
    for row in c:
        print(row)
def dolars_to_int(dolars):
    earned = re.findall(r'(\d+.?\d\d+.?\d *\d+.?\d *) *', dolars)
...
if __name__ == "__main__":
    ...
    movies_earned_over_100_000_000()
    whole_data=generate_data_from_api()
    whole_data=whole_data[['Title', 'Year', 'Runtime', 'Genre', 'Director', 'Actors', 'Writer', 'Language', 'Country', 'Awards','imdbRating', 'imdbVotes', 'BoxOffice']]
    print(whole_data)
